...
const NAV_ITEMS: readonly NavItem[] = [
  ...
  { label: "Layanan", href: "/#layanan", matchHash: "layanan" },
  { label: "Produk", href: "/#produk", matchHash: "produk" },
  { label: "Lokasi", href: "/#lokasi", matchHash: "lokasi" },
] as const;

function ConsultationChatIcon() {
  return (
    <svg
...
function navLinkClassName(isActive: boolean) {
  return isActive
    ...
    : "text-sm text-[#565d6d] transition-colors hover:text-primary";
}

type SiteHeaderDesktopProps = Readonly<{
  waHref: string | null;
}>;

function navTargetHash(item: NavItem) {
  return item.isHome ? "" : (item.matchHash ?? "");
}

export function SiteHeaderDesktop({ waHref }: SiteHeaderDesktopProps) {
  const pathname = usePathname();
  const hash = useLocationHash();
  const [pendingHash, setPendingHash] = useState<string | null>(null);

  useEffect(() => {
    const clearPending = () => setPendingHash(null);
    window.addEventListener("popstate", clearPending);
    return () => window.removeEventListener("popstate", clearPending);
  }, []);

  const effectivePending = pathname === "/" ? pendingHash : null;
  const activeHash =
    effectivePending !== null && hash !== effectivePending
      ? effectivePending
      : hash;

  const isNavActive = (item: NavItem) => {
    if (pathname !== "/") return false;
    if (item.isHome) return activeHash.length === 0;
    return item.matchHash === activeHash;
  };

  return (
    <header className="sticky top-0 z-50 hidden border-b border-[#dee1e6]/80 bg-white lg:block">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-0">
          ...
          <nav className="flex items-center gap-8" aria-label="Navigasi utama">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={navLinkClassName(isNavActive(item))}
                onClick={() => setPendingHash(navTargetHash(item))}
              >
                {item.label}
              </Link>
            ))}
        ...
        {/* <div className="flex items-center gap-6">
          {waHref ? (
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-primary px-6 py-2.5 text-sm font-bold text-white transition-colors hover:bg-primary-dark"
            >
            ...
            <span
              className="inline-flex cursor-not-allowed items-center gap-2 rounded-lg bg-primary/60 px-6 py-2.5 text-sm font-bold text-white"
