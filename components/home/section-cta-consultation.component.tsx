...
export default function SectionCtaConsultationComponent() {
  ...
  return (
    <section className="px-4 pb-4 pt-8 lg:max-w-7xl lg:mx-auto lg:px-0">
      <div className="relative overflow-hidden rounded-2xl bg-mate-black p-6 shadow-xl md:p-10 lg:rounded-[40px] lg:p-12">
        <div className="absolute right-0 top-0 h-full w-1/3 translate-x-1/4 -skew-x-12 bg-primary/20" />

        <div className="relative z-10 flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-center">
          <div className="max-w-xl">
            <h2 className="text-base mobile-m:text-lg font-bold leading-tight text-white md:text-2xl lg:text-4xl">
              Butuh Konsultasi
              <br className="hidden sm:block" />
              Kerusakan Laptop?
            </h2>
            <p className="mt-2 text-xxs mobile-m:text-xs text-white/70 md:text-sm lg:text-lg">
              Teknisi kami siap menjawab pertanyaan Anda via WhatsApp. Gratis,
          ...
          <div className="flex w-full flex-col items-stretch gap-3 sm:w-auto sm:items-center">
            {waHref ? (
              <Link
                ...
                className="inline-flex min-h-11 items-center justify-center gap-3 rounded-2xl bg-primary px-8 py-4 text-sm font-bold text-white active:scale-[0.98] lg:px-12 lg:text-lg"
              >
                <svg
              ...
              <p className="max-w-xs text-xs text-white/60">
                Pasang nomor pusat Ri Computer dalam `.env →
                SHOP_WHATSAPP_NUMBER` supaya tautan konsultasi bisa aktif.
              </p>
            )}
            <span className="text-xxs mobile-m:text-xs text-center text-white/50">
              Tersedia untuk bantuan servis
            </span>
          </div>
        </div>
      </div>
